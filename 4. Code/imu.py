...
class IMU:
    def __init__(self, SCL_pin: int, SDA_pin: int, address: hex) -> None:
        ...
        self.kf.x = np.zeros(6)   # Initial state: [accel_x, accel_y, accel_z, gyro_x, gyro_y, gyro_z]
        self.kf.P = np.eye(6)     # Initial state covariance

        # Set state transition matrix F, observation matrix H, process noise covariance Q, and measurement noise covariance R
        self.kf.F = np.eye(6)     # Identity matrix for state transition (simple integration)
        self.kf.H = np.eye(6)     # Identity matrix for observation (direct measurements)
    ...
    def read_data(self, reg: hex = 0) -> AccelGyroData:
        ...
        filtered_data = self.kf.predict_update([accel_x, accel_y, accel_z, gyro_x, gyro_y, gyro_z])
